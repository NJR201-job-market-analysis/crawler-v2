...
#(二)預設篩選條件, 請依需求自行修改
looking_for="desc&ks="+urllib.parse.quote("軟體+工程師") #查找中文關鍵字, 中間用+連接
...
#(三)查找資料入口
find_all_pages(looking_for,p_start,p_limit,w_place,wfh,ex)
df = pd.json_normalize(table)
print("找到幾個職缺: ",df.shape[0]) #回傳找到幾個職缺

#(四)用class創資料夾並存檔案
f1=File(name="1111jobs.csv") # name="檔名"
